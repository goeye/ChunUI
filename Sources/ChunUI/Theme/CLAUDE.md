...
CCModifiers.swift: 系统效果兼容边界（iOS 26 Liquid Glass / 18.6–25 微拟物同形降级：ccGlassEffect/softGlassStyle；SoftGlassShape 含 .custom(AnyShape) 任意轮廓，全部落成单一可动画 Shape 类型 SoftGlassPath——宿主切形不换身份、圆角可插值，禁按 case 分支给 glassEffect 传不同 Shape 类型：那会让含 UITextView 的子树重建、键盘二次弹出）+ appleCard/ccGroupCard/shimmer
